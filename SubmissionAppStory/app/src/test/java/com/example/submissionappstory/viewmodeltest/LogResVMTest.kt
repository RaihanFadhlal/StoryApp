...
class LogResVMTest {
    ...
    @Mock private lateinit var accountRepo: AccountRepository
    ...
    fun loginSuccess() = runTest {
        ...
        `when`(accountRepo.login(dummyEmail, dummyPass)).thenReturn(expectedData)

        val actualData = logresviewModel.login(dummyEmail, dummyPass).getOrAwait()
        verify(accountRepo).login(dummyEmail, dummyPass)
    ...
    fun registerSuccess() = runTest {
        ...
        `when`(accountRepo.register("dummytest", dummyEmail, dummyPass)).thenReturn(expectedData)

        val actualData = logresviewModel.register("dummytest",dummyEmail, dummyPass).getOrAwait()
        verify(accountRepo).register("dummytest", dummyEmail, dummyPass)
        Assert.assertNotNull(actualData)
        assertEquals(expectedData.value, actualData)
    }
}